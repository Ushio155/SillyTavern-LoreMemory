...
/** 一个折叠区块 */
function drawer(key, ctx, bodyHtml, { badge = '' } = {}) {
    ...
    return `
    <div class="inline-drawer lm-drawer" data-lm-drawer="${key}">
...
function head(ctx) {
    const { settings, bookName } = ctx;
    return `
    <div class="lm-head">
        <div class="lm-title">
            <i class="fa-solid fa-brain"></i> 记忆节点 · LoreMemory
            <span class="lm-badge-demo">DEMO</span>
            ${settings.autoMemory ? '<span class="lm-badge lm-badge-auto">自动记忆</span>' : ''}
        </div>
        <div class="lm-book" title="记忆条目所在的聊天世界书">
            ${bookName ? `<code>${escapeHtml(bookName)}</code>` : '<span class="lm-dim">未绑定世界书</span>'}
        </div>
    </div>`;
}
...
function statusBar(ctx) {
    ...
    const overBudget = maxContext > 0 && pct > Number(settings.budgetWarnPct || 10);
